...
# mutate(spend_perlab = scales::label_number(big.mark = ".")(round(spend_per)))
...
charts <- age_targeting %>%
  ...
  map( ~ {
    ...
    hchart(.x,
      ...
      hc_yAxis(title = list(text = "Budget voor targeting (% van totaal)"), max = 100, min = 0) %>%
      hc_xAxis(title = "", labels = list(style = list(fontSize = "10px"))) %>%
      hc_add_theme(hc_theme_google()) %>%
      highcharter::hc_tooltip(
        pointFormat = paste0(
"<b>{point.age_groups}</b><br>% van totaal budget besteed: {point.y:.1f}%<br>in totaal ",
currency_symbol,
"{point.spend_perlab} aan deze leeftijdsgroep"
        ),
        headerFormat = ""
      ) %>%
      hc_title(
...
charts <- gender_targeting %>%
  ...
  map(~ {
    ...
    hchart(.x, type = "bar", hcaes(x = value, y = perc), color = the_color) %>%
      hc_plotOptions(bar = list(horizontal = TRUE, stacking = "normal")) %>%
      hc_yAxis(title = list(text = "Budget voor targeting (% van totaal)"),
      ...
      hc_add_theme(hc_theme_google()) %>%
      highcharter::hc_tooltip(
        pointFormat = paste0("<b>{point.value}</b><br>% van totaal budget besteed: {point.y:.1f}%<br>in totaal ", currency_symbol, "{point.spend_perlab} aan deze gender"),
        headerFormat = ""
      ) %>%
...
charts <- lang_targeting %>%
  ...
  map(~ {
    ...
    hchart(.x, type = "bar", hcaes(x = value, y = perc), color = the_color) %>%
      ...
      hc_add_theme(hc_theme_google()) %>%
      highcharter::hc_tooltip(
        pointFormat = paste0("<b>{point.value}</b><br>% van totaal budget besteed: {point.y:.1f}%<br>in totaal ", currency_symbol, "{point.spend_perlab} aan deze taal"),
        headerFormat = ""
      ) %>%
      hc_title(text = unique(.x$party), align = "left", style = list(fontWeight = "bold")) %>%
      hc_size(height = 300) %>%
...
charts <- job_targeting %>%
  ...
  map(~ {
    ...
    hchart(.x, type = "bar", hcaes(x = value, y = perc), color = the_color) %>%
      ...
      hc_add_theme(hc_theme_google()) %>%
      highcharter::hc_tooltip(
        pointFormat = paste0("<b>{point.value}</b><br>% van totaal budget besteed: {point.y:.1f}%<br>in totaal ", currency_symbol, "{point.spend_perlab} aan deze baan/opleiding (sector)"),
        headerFormat = ""
      ) %>%
      hc_title(text = unique(.x$party), align = "left", style = list(fontWeight = "bold")) %>%
      hc_size(height = 300) %>%
