...
How much did the campaigns of political parties spend on different focus groups? The charts below show both the total and the expenditures per party.
...
saveRDS(col_each30, "col_each30.rds")
  ...
  col_each30_excl <- election_dat30 %>%
    ...
    group_split(internal_id) %>%
    map_dfr(~{
        calc_targeting(.x, exclude = T) %>%
            mutate(#coalition = .x$coalition[1],
                   party = .x$party[1],
                   internal_id = .x$internal_id[1])
    })
  
  saveRDS(col_each30_excl, "col_each30_exclrds")
