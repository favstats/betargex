...
Ontdek hoeveel partijen en politici al hebben uitgegeven aan advertenties op Facebook en Instagram sinds de start van de sperperiode
...
thedata <- readRDS("endit.rds") %>% 
  ...
  filter(!(party %in% c("VU", "l'Unie", "B.U.B.", "ProDG", "Viva Palestina", "Voor U", "DierAnimal")))
...
highchart() %>%
  ...
  hc_add_series(
    ...
    name = "Totale Uitgaven in € (sinds 9 februari)",
...
findat <- interest_targeting30 %>% 
  ...
  mutate(perc = perc/100, 
         perc_ads = perc_ads/100) %>% 
    filter(target %in% c("Gender: Mannen", "Gender: Vrouwen", "age", "language")) %>% 
  ...
  # filter(n == 3) %>%
  mutate(detailed_type = case_when(
  target == "custom_audience" ~ "DOELGREOPEN",
  target == "regions" ~ "GEOGRAFIE",
  target == "lookalike_audience" ~ "DOELGREOPEN",
  target == "interest" ~ "Gedetailleerd",
  target == "age" ~ "DEMOGRAFIE",
  target == "zips" ~ "GEOGRAFIE",
  target == "CITY" ~ "GEOGRAFIE",
  target == "language" ~ "DEMOGRAFIE",
  target == "gender" ~ "DEMOGRAFIE",
  target == "Gender: Mannen" ~ "DEMOGRAFIE",
  target == "Gender: Vrouwen" ~ "DEMOGRAFIE",
  target == "COMUNE" ~ "GEOGRAFIE",
  target == "electoral_districts" ~ "GEOGRAFIE",
  target == "COUNTY" ~ "GEOGRAFIE",
  str_detect(target, "NEIGHBOR") ~ "GEOGRAFIE",
  target == "MUNICIPALITY" ~ "GEOGRAFIE",
    T ~ detailed_type
  )) %>% 
...
yes <<-  election_dat30 %>%
  ...
  set_names(c("Pagina", "Advertenties", "Uitgegeven (€)", "internal_id"))
...
styled_reactable <- yes %>%
  ...
  reactable(
    ...
    columns = list(
      ...
      `Uitgegeven (€)` = colDef(
        header = with_tooltip("Uitgegeven (€)", last30days_string),
        defaultSortOrder = "desc", 
        cell = data_bars(
          data = .,
              fill_color = "#3fc1c9",
...
yes <<-  election_dat30 %>%
  ...
  set_names(c("Pagina", "Advertenties", "Uitgegeven (€)", "Race", "internal_id")) 
...
<div id="Viva Palestina" class="party-content" style="display:none;">
```{r}
make_tab("Viva Palestina")
```
</div>

<div id="Voor U" class="party-content" style="display:none;">
```{r}
make_tab("Voor U")
```
</div>

<div id="l'Unie" class="party-content" style="display:none;">
```{r}
make_tab("l'Unie")
```
</div>

<div id="B.U.B." class="party-content" style="display:none;">
```{r}
make_tab("B.U.B.")
```
</div>

<div id="ProDG" class="party-content" style="display:none;">
```{r}
make_tab("ProDG")
```
</div>

<div id="PB" class="party-content" style="display:none;">
```{r}
make_tab("Blanco")
```
</div>
