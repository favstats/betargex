...
Tijdens deze periode hebben politieke partijen in totaal `r paste0(currency_symbol, scales::comma_format()(sum30$total_spend_formatted))` uitgegeven en `r sum30$total_num_ads` advertenties getoond op Meta.
...
the_table30 <- get_table_dat(election_dat30, party) %>%
  arrange(desc(parse_number(`Totale uitgave`)))
...
Tijdens deze periode hebben politieke partijen in totaal `r paste0(currency_symbol, scales::comma_format()(sum7$total_spend_formatted))` uitgegeven en `r sum7$total_num_ads` advertenties getoond op Meta.
